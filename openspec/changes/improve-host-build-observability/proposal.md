...
- Improve all host-pipeline failure reports by reusing the existing bounded redacted diagnostic tail once per final report, explicitly labeled as retained context that may repeat live output. Do not reconcile retained diagnostics with live delivery history; normal live deduplication and coalescing remain supported but are best effort under presentation degradation.
- Simplify the current internal presentation path to one bounded ordered inbox and one facade actor, with a fixed 1024-event control capacity as an intentional hard safety limit, short mutex-protected admission, bounded drop accounting, and capacity-independent inbox close. The limit provides substantial headroom rather than encoding the exact current build transcript; exact production event sequences remain independently tested. Keep collection and retained tails independent of terminal throughput; preserve the optional external SDK sink contract.
- Bound presentation completion to one shared five-second budget across draining, acknowledgement, and join. A stalled or failed renderer must not change or indefinitely delay the primary build result; after that budget, cancel further presentation without synchronous retry to the failed stream. Clean terminal handoff and worker termination remain normal-path guarantees, not promises for indefinitely blocked I/O.
...
- Requires deterministic clock/wait injection and tests for FIFO admission ordering, producer-before-terminal completion, capacity-independent close, one-budget shutdown, stalled-renderer isolation, heartbeat timing, redaction, URL sanitization, exception-chain bounds, output-mode isolation, transient rendering, cancellation, and labeled failure-tail reuse. Replace tests tied to physical queue lanes, lock-contention emergency stops, exact delivery reconciliation, and unconditional worker termination; retain normal-path coalescing coverage.
- This is a revision of the current Phase 9 implementation, not a rewrite of completed collection, activity, configuration, or npm policy work. It does not introduce a combined heartbeat/presentation coordinator, asyncio migration, nonblocking terminal backend, or presentation subprocess.
